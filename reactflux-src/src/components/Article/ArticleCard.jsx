 ...
 * Handles the display of translated titles with optional bilingual mode
 */
const TitleDisplay = ({ entry, getTitleDisplay, titleRef }) => {
  const { displayTitle, showOriginal, originalTitle, isTranslating } = getTitleDisplay(
    entry.id,
    entry.title
  )

  return (
    <div className="card-title-wrapper" ref={titleRef}>
      <h3 className="card-title">
        {displayTitle}
        {isTranslating && (
          <IconLoading className="card-title-loading" />
        )}
      </h3>
      {showOriginal && originalTitle && (
        <p className="card-title-original">{originalTitle}</p>
      )}
    </div>
  )
}
...
const ArticleCard = ({ entry, handleEntryClick, children }) => {
  ...
  // Title translation
  const { observeElement, unobserveElement, getTitleDisplay } = useTitleTranslation()
  ...
  useEffect(() => {
    if (titleRef.current && entry.feed?.category?.id) {
      observeElement(
        titleRef.current,
        entry.id,
        entry.title,
    ...
    return () => {
      unobserveElement(entry.id)
    }
  }, [entry.id, entry.title, entry.feed?.category?.id, observeElement, unobserveElement])
      ...
      >
        ...
        >
          <div className="card-header">
            ...
            <TitleDisplay
              entry={entry}
              getTitleDisplay={getTitleDisplay}
              titleRef={titleRef}
            />
          </div>
