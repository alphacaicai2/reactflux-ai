...
const ArticleDetail = forwardRef((props, ref) => {
  ...
  // AI state
  const aiConfig = useStore(aiConfigState)
  ...
  // Reset summary visibility when article changes
  useEffect(() => {
    setShowSummary(true)
  }, [activeContent.id])
    ...
    >
      ...
      >
        <FadeTransition y={20}>
          ...
          {/* AI Summary Panel */}
          {isAIConfigured && aiConfig.enabled && showSummary && (
            <div style={{ maxWidth: getResponsiveMaxWidth(), margin: "0 auto" }}>
              <AISummaryPanel
                article={activeContent}
                visible={showSummary}
                autoGenerate={aiConfig.autoSummary}
