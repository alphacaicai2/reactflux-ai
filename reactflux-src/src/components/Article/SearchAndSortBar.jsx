...
import { DigestGenerateModal } from "@/components/Digest"
  ...
  return (
    <div className="search-and-sort-bar" style={{ width: isBelowMedium ? "100%" : 370 }}>
      ...
      <div className="button-group">
        ...
        {isAIConfigured && (
          <CustomTooltip mini content={polyglot.t("digest.generate")}>
            <Button
              icon={<IconFile />}
              shape="circle"
              size="small"
              onClick={() => setDigestModalVisible(true)}
            />
          </CustomTooltip>
        )}
      </div>
      ...
      <DigestGenerateModal
        ...
        onCancel={() => setDigestModalVisible(false)}
        onGenerate={() => setDigestModalVisible(false)}
      />
    </div>
  )
}
